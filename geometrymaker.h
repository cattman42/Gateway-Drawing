...
inline void getArchVbILen(int& vbLen, int& ibLen) {
  vbLen = 24;
  ibLen = 36;
}

template<typename VtxOutIter, typename IdxOutIter>
void makeArch(float size, VtxOutIter vtxIter, IdxOutIter idxIter) {
  float h = size / 2.0;
...
void makeArch(float start_t, unsigned short steps, float step_t, vMaker makeV, Cvec3f vertex, Cvec3f vertex1, Cvec3f norm, nMaker makeN, VtxOutIter vtxIter, IdxOutIter idxIter)
{
    ...
    for(int t = 0;t < tCount;t++)
    {
        ...
        Cvec3f vertexA = makeV(paramT, vertex);
        Cvec3f vertexB = makeV(paramT, vertex1);
        Cvec3f normal = makeN(paramT,norm);
        *vtxIter = SmallVertex(vertexA,normal);
        *idxIter = 2*t;
        ++vtxIter;
        ++idxIter;
        *vtxIter = SmallVertex(vertexB,normal);
        *idxIter = 2*t+1;
        ++vtxIter;
        ++idxIter;
    }
